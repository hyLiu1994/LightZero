...
mujoco_sampled_efficientzero_config = dict(
    exp_name=
    f'data_sez_ctree_pytest/{env_id[:-3]}_RobustZero_Random_cl_ns{num_simulations}_upc{update_per_collect}_rr{reanalyze_ratio}_bs-{batch_size}_pelw{policy_entropy_loss_weight}_seed{seed}',
    env=dict(
        env_id=env_id,
        action_clip=True,
        continuous=True,
