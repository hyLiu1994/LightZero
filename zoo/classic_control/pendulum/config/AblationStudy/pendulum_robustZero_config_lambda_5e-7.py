...
pendulum_sampled_efficientzero_config = dict(
    exp_name=
    f'data_sez_ctree_pendulum/pendulum_RobustZero_lambda{5e-7}_k{K}_ns{num_simulations}_upc{update_per_collect}_rr{reanalyze_ratio}_bs_{batch_size}_seed_{seed}',
    env=dict(
        env_id='Pendulum-v1',
        continuous=True,
    ...
    policy=dict(
        ...
        robustzero_w1=-1,
        optim_type='AdamAd',
        robustzero_lambda=5e-7,  # 0.0015  5e-6    5e-5 0.015 0.001
        # -------------------------------
        cuda=True,
