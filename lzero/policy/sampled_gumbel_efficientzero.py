...
def get_policies(discount_factor, k, action_space_size):
    mu = torch.tensor([random.random() for _ in range(15)])
    sigma = torch.tensor([random.random() for _ in range(15)])
    dist = Independent(Normal(mu, sigma), 1)
    samples = dist.sample((action_space_size,))
    sampled_actions_before_tanh = dist.sample(torch.tensor([k]))  # [20,3]
    sampled_actions = torch.tanh(sampled_actions_before_tanh)
    return sampled_actions


@POLICY_REGISTRY.register('sampled_gumbel_efficientzero')
class SampledGumbelEfficientZeroPolicy(MuZeroPolicy):
    """
    Overview:
    ...
    def _forward_collect(
        ...
        with torch.no_grad():
            ...
            roots_improved_policy_probs = get_policies(self._cfg.discount_factor, self._cfg.model.num_of_sampled_actions,
            ...
            for i, env_id in enumerate(ready_env_id):
                ...
                output[env_id] = {
                    ...
                    'improved_policy_probs': roots_improved_policy_probs
                }
