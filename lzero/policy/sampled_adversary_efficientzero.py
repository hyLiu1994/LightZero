...
from lzero.policy import scalar_transform, InverseScalarTransform, cross_entropy_loss, phi_transform, \
    ...
    configure_optimizers, prepare_obs_and_true_obs
...
class SampledAdversaryEfficientZeroPolicy(MuZeroPolicy):
    ...
    def _forward_learn(self, data: torch.Tensor) -> Dict[str, Union[float, int]]:
        ...
        # ==============================================================
        obs_batch_ori, true_obs_batch_ori, action_batch, child_sampled_actions_batch, mask_batch, indices, weights, make_time = current_batch
        target_value_prefix, target_value, target_policy = target_batch

        # obs_batch, obs_target_batch = prepare_obs(obs_batch_ori, self._cfg)
        obs_batch, true_obs_batch, obs_target_batch = prepare_obs_and_true_obs(obs_batch_ori, true_obs_batch_ori, self._cfg)
        ...
        if self._cfg.use_augmentation:
            ...
            if self._cfg.model.self_supervised_adversary_learning_loss:
                true_obs_batch = self.image_transforms.transform(true_obs_batch)
        ...
        for step_k in range(self._cfg.num_unroll_steps):
            ...
            if self._cfg.model.self_supervised_adversary_learning_loss:
                ...
                if self._cfg.c3 > 0:
                    ...
                    # NOTE: no grad for the representation_state branch.
                    if step_k == 0:
                        obs_proj = self._learn_model.project_robust(obs, with_grad=True)
                        true_obs_proj = self._learn_model.project_robust(true_obs, with_grad=False)
